 ...
 */
public class CameraPreviewActivity extends AbsCameraPreviewActivity implements
	...
	private void initPreFrame() {
		...
		Log.d(TAG, "PreSummeryEditorActivity.sBitmapList.size() "
				+ PreSummeryEditorActivity.sBitmapList.size());
		...
		for (AnimDrawableItem b : PreSummeryEditorActivity.sBitmapList) {
			mPreDrawable.addFrame(
					new BitmapDrawable(getResources(), b.getBitmap()), 3000);
		}
	}
	...
	Animation.AnimationListener mAnimListener = new Animation.AnimationListener() {
		...
		public void onAnimationEnd(Animation animation) {
			Log.d(TAG, "onAnimationEnd " + count);
			count++;
			if (count > 4) {
				count = 0;
				...
				toggleCapture(false);
				captureButton
						.setBackgroundResource(R.drawable.camera_recording__stop_btn);
				return;
			}
			mImageViewTimer.setImageLevel(count);
			playTimer();
	...
	private void playTimer() {
		...
		set.addAnimation(AnimationUtils.loadAnimation(
				CameraPreviewActivity.this, R.anim.camera_record_set));
	...
	private void setViewVisible(int show) {
		...
		mHandler.sendMessage(mHandler.obtainMessage(show,
				findViewById(R.id.buttonSettings_text)));
		mHandler.sendMessage(mHandler.obtainMessage(show,
				findViewById(R.id.setPreFrame)));
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if (progress >= MAX_TIME_IN_MS) {
							...
							if (isRecording() && mMediaEngine != null
									&& !mMediaEngine.isPrepared())
								progress = 0;
				...
				try {
					...
					if (b != null) {
						...
						BitmapUtils.saveBitmap(
								mediaFilePath.substring(0,
										mediaFilePath.lastIndexOf(".") + 1)
										.concat("png"), b);
					}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case REQ_PREFRAME:
			...
			((TextView) findViewById(R.id.setPreFrame_text))
					.setText(R.string.modifypreframe);
			initPreFrame();
			break;
		}
	}
	...
	public void onClick(@NonNull View v) {
		...
		case R.id.setPreFrame:
			...
			mPreFrameIntent.putExtra("musicname",
					getIntent().getStringExtra("musicname"));
		...
		case R.id.buttonSettings:
			...
			if(mAvator == 1)
				...
				Bundle b = new Bundle();
				b.putInt("avator", mAvator==2?BitmapGridFragment.LIST_TYPE_AVATOR2:BitmapGridFragment.LIST_TYPE_AVATOR3);
	...
	@Override
	public void onItemClick(AnimItem fgId) {
		Drawable fg = fgId.getDrawable(this);
	...
	protected void updateTorchView(boolean bshow) {
		findViewById(R.id.torch).setVisibility(
				bshow ? View.VISIBLE : View.INVISIBLE);
			...
			public void run() {
				...
				case NORMAL:
					...
					mHandler.sendMessageDelayed(
							mHandler.obtainMessage(HIDE_FOCUS, mFocusIndicator),
