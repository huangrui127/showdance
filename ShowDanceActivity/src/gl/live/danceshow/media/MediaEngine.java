...
public class MediaEngine implements SurfaceTexture.OnFrameAvailableListener, OnFrameUpdateListener {
    ...
    private static boolean bDEBUG = false; // log开关
    ...
    public boolean initAudio(String file) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mime.startsWith("audio")) {
                channelcount = mFormat.getInteger(MediaFormat.KEY_CHANNEL_COUNT);
        ...
        L.i(TAG, "samplerate " + samplerate + " channelcount " + channelcount + " mime " + mime + " bitrate " + br
                + " magicLag " + magicLag);
        ...
        L.d(TAG, "mAudioMinBufSize " + mAudiotrackMinBufSize);
        mAudioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, samplerate,
                channelcount == 2 ? AudioFormat.CHANNEL_OUT_STEREO : AudioFormat.CHANNEL_OUT_MONO,
                AudioFormat.ENCODING_PCM_16BIT, mAudiotrackMinBufSize, AudioTrack.MODE_STREAM);
    ...
    public boolean initVideo(String path, String type) throws IOException {
        ...
        mVideoMuxer = new MediaMuxer(path, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
        return true;
    }

    public void setPreviewDisplay(SurfaceHolder surface) {
        mVideoFormat = MediaFormat.createVideoFormat("video/avc", PreviewManager.EXPECTED_PREVIEW_W,
                PreviewManager.EXPECTED_PREVIEW_H);
        ...
        L.i(TAG, " mVideoFormat " + mVideoFormat.toString());
    ...
    private class AudioDecode implements Runnable {
        ...
        public void run() {
            ...
            long pts = 0;
            if (samplesize < 0) {
                L.d(TAG, "audio decode samplesize " + samplesize + "in[" + index + "] " + in[index].capacity());
                bEos = true;
                samplesize = 0;
            } else {
                bEos = false;
            ...
            mAudioDecoder.queueInputBuffer(index, 0, samplesize, pts, bEos ? MediaCodec.BUFFER_FLAG_END_OF_STREAM : 0);
    ...
    Runnable mAudioDecodeOutputEvent = new Runnable() {
        ...
        public void run() {
            ...
            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                if (!bStop) {
                    L.w(TAG, "audio Decode reached end of audio stream unexpectedly");
                    ...
                    L.d(TAG, "audio Decode end of stream reached");
                }
            } else if (index == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                L.w(TAG, "audio Decode  INFO_OUTPUT_BUFFERS_CHANGED");
                out = mAudioDecoder.getOutputBuffers();
            } else if (index == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                MediaFormat format = mAudioDecoder.getOutputFormat();
                L.w(TAG, "audio Decode  INFO_OUTPUT_FORMAT_CHANGED " + format.getString(MediaFormat.KEY_MIME));
            } else if (info.flags == MediaCodec.INFO_TRY_AGAIN_LATER) {
                L.w(TAG, "audio Decode  INFO_TRY_AGAIN_LATER");
            } else if (index < 0) {
                // L.w(TAG, "audio Decode index "+index);
            } else {
    ...
    private class PlayTrack implements MyRunnable {
        ...
        public void run() {
            ...
            if (buf.length > 0) {
                ...
                if (bDEBUG)
                    L.w(TAG, "audio track write begin " + Build.MODEL + " buf.length " + buf.length);
                long time = System.nanoTime() / 1000;
                if (bfirst) {
                    bfirst = false;
                    audioStartTime = time;
                    if (bDEBUG)
                        L.w(TAG, "bAudioFirst " + time);
                }
                ...
                if (isvideoready) {
                    ...
                    if (bDEBUG)
                        L.w(TAG, "video is not ready yet....");
                    audioStartTime = time;
                    mAudioPlayHandler.post(mAudioDecodeOutputEvent);
                    return;
                }
                ...
                if (bDEBUG)
                    L.w(TAG, "audio track write end ---- 结束写入时间："+((System.nanoTime() - time)/1000000) +"毫秒");
                AudioEncodeEvent mAudioEncode = new AudioEncodeEvent(time);// System.nanoTime()
                                                                           // -
                                                                           // (buf.length/2304)
                                                                           // *10);
    ...
    private class AudioEncodeEvent implements MyRunnable {
        ...
        public void run() {
            ...
            if (index >= 0) {
                // L.w(TAG,"mAudioEncode dequeueInputBuffer-------------- "
                // +index+ " mBuffer length "+mBuffer.length);
                ByteBuffer inputBuffer = in[index];
                inputBuffer.clear();

                long presentationTimeUs = pts;// (System.nanoTime() -
                                              // audioStartTime) / 1000;
                if (bDEBUG)
                    L.w(TAG, "audio encoder  pts = " + presentationTimeUs);
                if (mBuffer != null) {
                    inputBuffer.put(mBuffer);
                    mAudioEncoder.queueInputBuffer(index, 0, mBuffer.length, presentationTimeUs, 0);
                } else {
                    mAudioEncoder.queueInputBuffer(index, 0, 0, presentationTimeUs,
                            MediaCodec.BUFFER_FLAG_END_OF_STREAM);
                }

            } else {
                L.w(TAG, "mAudioEncode error " + index);
            }
        }
    ...
    private void handleAudioMux() {
        ...
        while (true) {
            ...
            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                // if (!bStop) {
                // L.w(TAG, "reached end of audio stream unexpectedly");
                ...
                synchronized (this) {
                    if (!bStop) {
                        ...
                        mContext.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                ((CameraPreviewActivity) mContext).toggleCapture(true);
                            }
                        });
                    }
                }
                ...
                L.d(TAG, "audio encoder output format changed: " + newFormat);
                ...
                // L.w(TAG, "audio encoder ---------- index "+index);
    ...
    private void handleVideoMux(long pts) {
        ...
        int encoderStatus = mVideoEncoder.dequeueOutputBuffer(mBufferInfo, 0);
        ...
        if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
            ...
            L.w(TAG, "video encoder output format changed: " + newFormat);
            ...
            L.w(TAG, "video unexpected result from encoder.dequeueOutputBuffer: " + encoderStatus);
            return;
            // let's ignore it
        } else {
            ...
            if (bVideoFirst) {
                bVideoFirst = false;
                videoStartTime = tmppts;// pts;// - Math.max(pts,
                                        // audioStartTime);
                L.w(TAG, "bVideoFirst   videoStartTime " + videoStartTime + " mVideoDelayTime " + mVideoDelayTime);
            }

            tmppts = tmppts - (videoStartTime - audioStartTime) - mVideoDelayTime;// -videoStartTime;//
                                                                                  // audioStartTime
                                                                                  // /
                                                                                  // 1000;//
                                                                                  // -
                                                                                  // mVideoDelayTime;
            if (bDEBUG)
                L.e("guolei", "video sent " + mBufferInfo.size + " bytes to muxer video  pts " + tmppts);

            ByteBuffer encodedData = encoderOutputBuffers[encoderStatus];
            if (encodedData == null) {
                throw new RuntimeException("video encoderOutputBuffer " + encoderStatus + " was null");
            }
            ...
            if (mBufferInfo.size != 0) {
                ...
                // L.e(TAG, "video sent " + mBufferInfo.size + " bytes to muxer
                // pts " + mBufferInfo.presentationTimeUs
                // + " audioStartTime " + audioStartTime + " videoStartTime " +
                // videoStartTime);
            }
    ...
    // buf = Bitmap.createBitmap(buf, 0, 0, outputBitmap.getWidth(),
    // outputBitmap.getHeight(), mBitmapMatrix, true);
    ...
    //// b = Bitmap.createBitmap(b, 0, 0, PREVIEW_WIDTH, PREVIEW_HEIGHT,
    // preMatrix, true);
    //// }
    // }
    // if(b!=null&& !b.isRecycled()){
    ...
    // canvas.drawText(text, canvas.getWidth()/2-mPaint.measureText(text)/2,
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            synchronized (MediaEngine.this) {
                ...
                if (mCamera == null)
                    return;
                if (bDEBUG)
                    L.w(TAG, "onPreviewFrame " + mVideoDelayTime);
    ...
    private Bitmap convertYuvImageToBitmap(Context context, byte[] data, int w, int h) {
        ...
        if (yuvAllocation == null || yuvAllocation.getBytesSize() < data.length) {
            ...
            L.w(TAG, "allocate in " + yuvAllocation.getBytesSize() + " " + w + "x" + h);
        }
        ...
        if (rgbaAllocation == null
            ...
            L.w(TAG, "allocate out " + rgbaAllocation.getBytesSize() + " " + w + "x" + h);
        }
