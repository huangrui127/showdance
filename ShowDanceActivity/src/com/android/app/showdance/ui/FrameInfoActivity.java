...
//			File lrcFile = new File(InitApplication.SdCardLrcPath.concat(musicItem.get("name").toString().replace(".mp3", ".ass")));
...
//			}
			...
			if (FileUtil.isFileExist(InitApplication.SdCardMusicPath, musicItem.get("name").toString())) {
				CameraPreviewActivity.actionRecord(this, REQUEST_RECORD, InitApplication.SdCardMusicPath.concat(musicItem.get("name").toString()), musicItem.get("name").toString().replace(".mp3", ""));
			} else {
...
//			getmp4PathFromSD(downMusicItem.getName());
			...
			// 自定义有标题、有确定按钮与有取消按钮对话框使用方法
			CustomAlertDialog mCustomDialog = new CustomAlertDialog(FrameInfoActivity.this).builder(R.style.DialogTVAnimWindowAnim);
			...
			mCustomDialog.setPositiveButton(getResources().getString(R.string.dialog_ok), new OnClickListener() {
				...
				public void onClick(View v) {
					...
					mIntent.setClass(FrameInfoActivity.this, VideoViewPlayingActivity.class);
