...
public class FrameInfoActivity extends BaseActivity implements OnItemClickListener {
    ...
    protected void findViewById() {
        ...
        DownloadFrameList = (ListView) findViewById(R.id.downloadedMusicList);
        no_login_layout = (LinearLayout) findViewById(R.id.no_login_layout);
    }

    @Override
    protected void initView() {
        tvTitle.setText("已下载边框");
        return_imgbtn.setVisibility(View.VISIBLE);
        inflater = LayoutInflater.from(this);
        mDownloadedFrameList = new ArrayList<Map<String, Object>>();
    ...
    protected void setOnClickListener() {
        ...
        DownloadFrameList.setOnItemClickListener(this);
    }
    ...
    private class ScanVideoTask extends AsyncTask<Void, File, List<Map<String, Object>>> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            pd = new ProgressDialog(FrameInfoActivity.this);
        ...
        protected void onPostExecute(List<Map<String, Object>> result) {
            ...
            mDownloadedFrameList = result;

            if (mDownloadedFrameList != null && mDownloadedFrameList.size() != 0) {
                DownloadFrameList.setVisibility(View.VISIBLE);
                no_login_layout.setVisibility(View.GONE);
                mDownloadedMusicAdapter = new DownloadedMusicAdapter(FrameInfoActivity.this, mDownloadedFrameList);
                DownloadFrameList.setAdapter(mDownloadedMusicAdapter);
            } else {
                DownloadFrameList.setVisibility(View.GONE);
                no_login_layout.setVisibility(View.VISIBLE);
            }
    ...
    protected void handleReceiver(Context context, Intent intent) {
        ...
        if (intent.getAction().equals(ConstantsUtil.ACTION_DOWN_MEDIARECORDER)) {
            ...
            // File lrcFile = new
            // File(InitApplication.SdCardLrcPath.concat(musicItem.get("name").toString().replace(".mp3",
            // ".ass")));

            // 使用随机数uid重命名歌词并复制到新的相关目录
            // newLrcFile = new File(InitApplication.SdCardNewLrcPath +
            // UUID.randomUUID().toString() + ".ass");
            // try {
            ...
            if (FileUtil.isFileExist(InitApplication.SdCardMusicPath, musicItem.get("name").toString())) {
                CameraPreviewActivity.actionRecord(this, REQUEST_RECORD,
                        InitApplication.SdCardMusicPath.concat(musicItem.get("name").toString()),
                        musicItem.get("name").toString().replace(".mp3", ""));
            } else {
    ...
    // // 打开正在合成进度条
    // showSizeProgressDialog(DownloadedMusicActivity.this, totalSize,
    ...
    // CustomAlertDialog(DownloadedMusicActivity.this).builder(R.style.DialogTVAnimWindowAnim);
    ...
    // @Override
    // public void onClick(View v) {
    // VideoReviewActivity.actionReview(DownloadedMusicActivity.this,
    ...
    // Toast.makeText(DownloadedMusicActivity.this, "转换失败: " + exceptionMessage,
    ...
    public File getmp4PathFromSD(String newVideoName) {
        ...
        for (int i = 0; i < files.length; i++) {
            File file = files[i];
            if (FileUtil.checkIsMp4File(file.getPath())) {
                renameFileNewPath = new File(InitApplication.SdCardRecordedVideoPath
                        + newVideoName.replace(".mp3", "").concat("_").concat(timeStamp).concat(".mp4"));
        ...
        case TaskType.TS_MusicInfoPageList: // 已下载舞曲
            ...
            if (ConstantsUtil.NetworkStatus) {
                ...
                if (mMusicList != null && mMusicList.size() != 0) {
                    ...
                    mDownloadedFrameList.clear();
                    mDownloadedFrameList.addAll(mMusicList);

                    if (refreshType == 0) {
                        mDownloadedMusicAdapter = new DownloadedMusicAdapter(FrameInfoActivity.this,
                                mDownloadedFrameList);
                        DownloadFrameList.setAdapter(mDownloadedMusicAdapter);
    ...
    private void handleRecordVideoResult(final String videopath) {
        ...
        if (jointOutFile.exists() && jointOutFile.length() > 0) {
            ...
            // 自定义有标题、有确定按钮与有取消按钮对话框使用方法
            CustomAlertDialog mCustomDialog = new CustomAlertDialog(FrameInfoActivity.this)
            ...
            mCustomDialog.setPositiveButton(getResources().getString(R.string.dialog_ok), new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    // mIntent.setClass(FrameInfoActivity.this,
                    // VideoViewPlayingActivity.class);
                    // mIntent.setData(Uri.parse(videopath));
                    // startActivity(mIntent);

                    Intent intent = new Intent(FrameInfoActivity.this, PlayVideoActivity.class);
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, final int position, long arg3) {
        ...
        final String fileName = mDownloadedFrameList.get(position).get("name").toString();

        if (!FileUtil.isFileExist(InitApplication.SdCardMusicPath, fileName)) {
            Toast.makeText(this, "音乐文件未找到", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        mCustomDialog.setPositiveButton(getResources().getString(R.string.dialog_ok), new OnClickListener() {
            @Override
            public void onClick(View v) {
                File delFilePath = new File(InitApplication.SdCardMusicPath.concat(fileName));
                delFilePath.delete();
                mDownloadedFrameList.remove(position);
                mDownloadedMusicAdapter.notifyDataSetChanged();
            }
        }).setNegativeButton(getResources().getString(R.string.dialog_cancel), new OnClickListener() {
            @Override
