...
public class DownloadFrameInfo implements Serializable {
	...
	private String downloadAction = ConstantsUtil.ACTION_DOWNLOAD_STATE;
	private FrameInfo.Frame mMusicSearchResponse;

	public DownloadFrameInfo(FrameInfo.Frame resp) {
		mMusicSearchResponse = resp;
		if (FileUtil.isFileExist(InitApplication.sdCardForegroundPath, resp.getname())) {
			downloadState = ContentValue.DOWNLOAD_STATE_SUCCESS;
		}
	}
	public DownloadFrameInfo(FrameInfo.Frame resp,String sdcardpath) {
		mMusicSearchResponse = resp;
		if (FileUtil.isFileExist(sdcardpath, resp.getname())) {
			downloadState = ContentValue.DOWNLOAD_STATE_SUCCESS;
		}
	}
	...
	public String getAuthor() {
		return mMusicSearchResponse.getteacher();
	}

	public int getDownloadCount() {
		return 0;//mMusicSearchResponse.getDownload_volume();
	}
	...
	public boolean equals(Object o) {
		if (this == o)
			return true;
		if (!(o instanceof DownloadFrameInfo))
			return false;

		DownloadFrameInfo that = (DownloadFrameInfo) o;

		if (mMusicSearchResponse.getId() != that.getFrame().getId())
			return false;
	...
	public String getFileSize() {
		return "0";//mMusicSearchResponse.getSize();
	}
