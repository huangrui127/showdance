...
public class FoundNearManAdapter extends BaseAdapter {
	...
	public FoundNearManAdapter(Context context, List<Map<String, Object>> menulist) {
...
//		finalBitmap.display(holder.a_img, );
		...
		//是否是"队长"
		if (listItem.get("isTeamHeader").toString().equals("")) {
			holder.isTeamHeader_tv.setText(""); //不是队长
		}else {
			String isTeamHeader = "("+listItem.get("isTeamHeader").toString()+")";
			holder.isTeamHeader_tv.setText(isTeamHeader);
		}
		
		holder.dance_team_tv.setText(listItem.get("dance_team").toString());
		holder.dance_team_starLevel_tv.setText(listItem.get("team_starLevel").toString());
